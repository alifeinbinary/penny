...
_BASIC_FLOW_EXPECTED = (
    ...
    "\n"
    "When you just need to remember something — a fact, a price, a detail worth keeping "
    "— store it in a collection: find the most appropriate one that already exists (your "
    "memory list in the state section names them) and write there, or create a new "
    "collection for it (just a name and a description — that's all storage is). Storing a "
    "fact is a plain write, not a job: don't put a trigger, schedule, or notify on the "
    "collection — those describe recurring work, and recurring work is a skill (next).\n"
    "\n"
    ...
    "- collector-runs (log, 0 entries) — One entry per Collector cycle: the target + its "
